...
#include <QMessageBox>
#include <iostream>
#include "../include/data_collector/main_window.hpp"
#include <stdlib.h>
	...
	, qnode(argc,argv)
{
    ...
    QPixmap pix((ros::package::getPath("data_collector") + "/resources/images/no_image.png").c_str());
    ui.image_view->setPixmap(pix.scaled(512,512,Qt::KeepAspectRatio));
...
void MainWindow::ReadSettings() {
    QSettings settings("Qt-Ros Package", "data_collector");
...
void MainWindow::on_button_start_clicked(bool check ) {
	qnode.begin = true;
...
void MainWindow::on_button_breathing_stopped_clicked(bool check ) {
	qnode.breathing_stopped = true;
...
//    	QImage qim1((const uchar *) img.data, img.cols, img.rows, img.step, QImage::Format_Mono);
	...
	QPixmap pix = QPixmap::fromImage(qim1);
    	...
    	int width_view = (int) width/2.0;
	int height_view = (int) height/2.0;
